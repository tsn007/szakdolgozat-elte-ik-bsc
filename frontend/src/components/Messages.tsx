...
export function Messages() {
    ...
    const convoFromRental = urlRentalId
        ...
        : null;
    const activeConvo =
        urlConvoId || convoFromRental || (!isMobile && conversations?.length ? conversations[0].id : "");

    if (isLoading) {
        return <Text>Loading...</Text>;
    }

    const showSidebar = !isMobile || !activeConvo;
    const showChatWindow = !isMobile || activeConvo;

    const handleBackToList = () => {
        searchParams.delete("convo");
        searchParams.delete("rental");
        setSearchParams(searchParams);
    };

    return (
        <Flex gap="xl" h="calc(100vh - 100px)" p={isMobile ? "sm" : "md"}>
            {showSidebar && (
                <Paper
                    ...
                    maw={isMobile ? "100%" : 350}
                    style={{ flex: isMobile ? 1 : "0 1 350px" }}
                >
            ...
            {showChatWindow && (
                <Box style={{ flex: 1, minHeight: 0 }} h="100%" w="100%">
                    <ConversationWindow
                        key={activeConvo}
                        convId={activeConvo}
